...
for subpath in  /mnt/siemens-dicom/anon/IRC_9*FOUNDCOG; do
    ...
    for sespath in $subpath/*; do
        ...
        heudiconv -d "/mnt/siemens-dicom/anon/{subject}_RC_FOUNDCOG/${sesbasename}/Series_*/*.dcm" -c dcm2niix --overwrite -o ../bids -b -f heuristic.py -s $sub -ss $ses -q SLURM  --queue-args="--time=4:0:0"
        ses=$(($ses + 1))
    done
done